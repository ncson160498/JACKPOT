...
function App() {
  ...
  const { contract } = useContract(
    "0xaef53D34C334d07DBa5ee50EF081bC7f1AEc7FfF"
  );
  ...
  const handleErrorToast = (
    ...
    if (!errMessage && !toastId) toast.error(defaultMessage);
    if (!errMessage && toastId)
      toast.error(defaultMessage, {
        id: toastId,
        duration: 5000,
      });
    if (toastId && errMessage)
      toast.error(
        <>
          <p>{defaultMessage}</p>
          <a
            onClick={() => {
              toast(errMessage, { style: { padding: 10 } });
            }}
            style={{
              color: "#2a75ed",
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      handleErrorToast("Buy ticket failed!", notify, message);
    }
  };
